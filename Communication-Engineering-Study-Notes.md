...
| Block | Function |
|---|---|
| Information source | Produces the message (voice, music, picture) — not electrical yet |
| Input transducer | Converts the physical message into a time-varying **electrical** signal (e.g. microphone, camera) |
| Transmitter | Restricts frequency range, amplifies, and **modulates** — superimposes the message onto a high-frequency carrier |
| Channel | The medium the signal travels through (wire, cable, radio waves, space). Noise gets added here |
| Noise | Random, unwanted signal that corrupts the message in the channel |
| Receiver | Extracts the original message from the degraded received signal (demodulation) |
| Output transducer / Destination | Converts the electrical signal back to its original form (e.g. loudspeaker) |
...
- Baseband signal f = 10 kHz → height = (3×10⁸)/(4×10×10³) = **7,500 m (7.5 km)** → impossible to build
...
- **Noise limitation** — unavoidable; measured via **Signal-to-Noise Ratio (SNR)**.
- Trade-offs always exist between transmission time/power, SNR performance, equipment cost, and channel bandwidth/capacity.
...
- **Frequency deviation: Δf = K·V_m** (K = deviation sensitivity, V_m = peak amplitude of message)
...
- **Δf = K·Vm**, **mf = Δf/fm**, **Carson's Rule: B = 2(Δf+fm)**
