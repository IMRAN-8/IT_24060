...
For an LTI system: **Y(f) = H(f)·X(f)**, \|Y(f)\|=\|H(f)\|·\|X(f)\|, phases add.

> **⭐ Condition for distortionless transmission:** output y(t) = k·x(t−td) (scaled, delayed copy). Requires: |H(f)| = k (constant), θ(f) = −2πf·td (linear phase). Group delay: td = −(1/2π)(dθ/df)

## ⭐ Signal Distortion Over a Channel (asked in 2023)
| Type | What happens | Consequence |
|---|---|---|
| **Linear distortion** | Different frequencies of a signal are changed in amplitude or phase. No new frequencies created | Signal shape changes but stays in the same band; fixable with equalization |
| **Non-linear distortion** | System produces new frequencies not present in the original signal (y(t)=a₀+a₁g(t)+a₂g²(t)+...) | Creates brand-new harmonic components. **If two signals sit in adjacent bands, these new components can spill into and interfere with the neighbouring band** |
| **Multipath distortion** | Signal reaches the receiver through multiple paths due to reflection and scattering | Causes **frequency-selective fading, phase distortion, inter-symbol interference, and reduced signal quality**. Combated with **Automatic Gain Control (AGC)** |

## Energy/Power Spectral Density & Autocorrelation
- **Energy Spectral Density (ESD):** Ψg(f) = |G(f)|²
- **Power Spectral Density (PSD):** Sg(f), Pg = ∫Sg(f)df
- **Autocorrelation** Rg(τ) = ∫g(t)g(t−τ)dt — measures similarity between a signal and a delayed copy of itself. It's the convolution of g(t) with g(−t). Rg(τ) and the ESD/PSD form a **Fourier Transform pair**.
- **⭐ Why we need autocorrelation:** random/noise-like signals can't be described by one formula, so we can't FT them directly — but we CAN compute their autocorrelation, then FT *that* to get the PSD.
...
10. Modulation basics: what it is, why it's needed, antenna-height example, why carrier frequency > message frequency
